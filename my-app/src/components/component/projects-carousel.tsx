      ...
      <div className='flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-4'>
        <div className='flex-1 flex flex-wrap gap-2'>
          {filterOptions.map((option, index) => (
            <button
              key={index}
              className={`justify-start px-4 py-2 border rounded flex items-center transition-colors ${
                selectedFilters.includes(option)
                  ? 'bg-gray-100 dark:bg-slate-700 hover:bg-gray-300 border-green-500 dark:border-green-500 hover:border-red-500 dark:hover:border-red-500'
                  : 'bg-white dark:bg-slate-900 hover:bg-gray-300 dark:border-slate-500'
              }`}
